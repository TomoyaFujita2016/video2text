...
video_id = st.text_input(label="video_id(https://www.youtube.com/watch?v={video_id})", value="xlDGAr5FAvA")
run_subtitle = st.button("実行")


if run_subtitle:
    subtitle = get_subtitle(video_id, sentence_num=sentence_num)
    st.subheader("実行結果")
    st.text(subtitle)
